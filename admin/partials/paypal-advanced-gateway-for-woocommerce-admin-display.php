...
class PayPal_Advanced_Gateway_For_WooCommerce_Admin_Display extends WC_Payment_Gateway {
    ...
    public function checks() {
        ...
        if (!$this->loginid) {
            ...
            echo '<div class="error"><p>' . sprintf(__('Paypal Advanced error: Please enter your PayPal Advanced Account Merchant Login <a href="%s">here</a>', 'paypal-advanced-gateway-for-woocommerce'), admin_url('admin.php?page=wc-settings&tab=checkout&section=' . strtolower('WC_Paypal_Advanced'))) . '</p></div>';
        } elseif (!$this->resellerid) {

            echo '<div class="error"><p>' . sprintf(__('Paypal Advanced error: Please enter your PayPal Advanced Account Partner <a href="%s">here</a>', 'paypal-advanced-gateway-for-woocommerce'), admin_url('admin.php?page=wc-settings&tab=checkout&section=' . strtolower('WC_Paypal_Advanced'))) . '</p></div>';
        } elseif (!$this->password) {

            echo '<div class="error"><p>' . sprintf(__('Paypal Advanced error: Please enter your PayPal Advanced Account Password <a href="%s">here</a>', 'paypal-advanced-gateway-for-woocommerce'), admin_url('admin.php?page=wc-settings&tab=checkout&section=' . strtolower('WC_Paypal_Advanced'))) . '</p></div>';
        }
    ...
    public function relay_response() {
        ...
        if ($status == 'processing' || $status == 'completed') {
            ...
            if ($not_silentreq_debug)
                $this->log->add('paypal_advanced', sprintf(__('Redirecting to Thank You Page for order %s', 'paypal-advanced-gateway-for-woocommerce'), $order->get_order_number()));
        ...
        if (isset($_REQUEST['error']) && $_REQUEST['error'] == 'true' && $_POST['RESULT'] != 0) { //handle errors and declines
            ...
            if ($_POST['RESULT'] == 12 && $status != 'failed') {
                ...
                // Log the status
                if ($debug == 'yes')
                    $this->log->add('paypal_advanced', sprintf(__('Status has been changed to failed for order %s', 'paypal-advanced-gateway-for-woocommerce'), $order->get_order_number()));
            }
    ...
    function get_secure_token($order) {
        ...
        try {
            ...
            if ($this->debug == 'yes')
                $this->log->add('paypal_advanced', sprintf(__('Secured Token generation failed for the order %s with error: %s', 'paypal-advanced-gateway-for-woocommerce'), $order->get_order_number(), $e->getMessage()));
...
// End init_form_fields()
    ...
    public function process_payment($order_id) {
        ...
        try {
            ...
            if ($this->securetoken != "") {
                ...
                //Log
                if ($this->debug == 'yes')
                    $this->log->add('paypal_advanced', sprintf(__('Secured Token generated successfully for the order %s', 'paypal-advanced-gateway-for-woocommerce'), $order->get_order_number()));
    ...
    public function receipt_page($order_id) {
        ...
        if ($this->debug == 'yes')
            $this->log->add('paypal_advanced', sprintf(__('Browser Info: %s', 'paypal-advanced-gateway-for-woocommerce'), $_SERVER['HTTP_USER_AGENT']));
        ...
        if ($this->layout == 'MINLAYOUT' || $this->layout == 'C') {
            ...
            //Log
            if ($this->debug == 'yes')
                $this->log->add('paypal_advanced', sprintf(__('Show payment form(IFRAME) for the order %s as it is configured to use Layout C', 'paypal-advanced-gateway-for-woocommerce'), $order->get_order_number()));
            ...
            //Log
            if ($this->debug == 'yes')
                $this->log->add('paypal_advanced', sprintf(__('Show payment form redirecting to ' . $location . ' for the order %s as it is not configured to use Layout C', 'paypal-advanced-gateway-for-woocommerce'), $order->get_order_number()));
